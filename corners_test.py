...
def corner_score(image, u=5, v=5, window_size=(5, 5)):
    ...
    corner_score = convolve(diff, window, mode='constant', cval=0.0)
...
def harris_detector(image, window_size=(5, 5)):
    ...
    Ix = convolve(image, np.array([[-1, 0, 1]]), mode='constant', cval=0.0)
    Iy = convolve(image, np.array([[-1], [0], [1]]), mode='constant', cval=0.0)
    ...
    window = np.ones(window_size)
    Sxx = convolve(Ixx, window, mode='constant', cval=0.0)
    Syy = convolve(Iyy, window, mode='constant', cval=0.0)
    Sxy = convolve(Ixy, window, mode='constant', cval=0.0)
